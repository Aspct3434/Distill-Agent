...
function generateEnv(config) {
  ...
  return [
    ...
    envLine("AGENT_MAX_TOKENS", "4096"),
    envLine("AGENT_PLANNING_MAX_TOKENS", "2048"),
    envLine("AGENT_ARTIFACT_MAX_TOKENS", "20000"),
    envLine("AGENT_FINAL_MAX_TOKENS", "4096"),
...
async function startLocalServices(config) {
  ...
  // uv installs the heavy ML wheels (torch, transformers, etc.) in parallel with a
  // global cache — far faster than pip. Prefer a system uv; otherwise bootstrap it
  // into the venv (tiny wheel); fall back to plain pip if uv can't be installed.
  const deps = ["pip", "install", "-r", "requirements.txt", "--python", venvPython];

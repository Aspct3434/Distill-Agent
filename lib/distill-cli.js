...
const DEFAULT_REPO_URL = "https://github.com/Aspct3434/Distill-Agent.git";
const DEFAULT_BRANCH = "master";
const CONFIG_DIR = path.join(os.homedir(), ".distill");
const CONFIG_FILE = path.join(CONFIG_DIR, "config.json");
const RUN_STATE_FILE = path.join(CONFIG_DIR, "run-state.json");

const PROVIDERS = ["kimi", "ollama", "openrouter", "openai", "anthropic", "gemini", "deepseek", "groq", "xai", "mistral", "vllm"];
...
const MODEL_CHOICES = {
  ...
  openai: [
    ...
    { label: "GPT-5.1", value: "gpt-5.1", note: "newest" },
    { label: "GPT-5", value: "gpt-5" },
    { label: "GPT-5 mini", value: "gpt-5-mini", note: "cheaper, faster" },
  ...
  deepseek: [
    { label: "DeepSeek Chat (V3)", value: "deepseek/deepseek-chat", note: "default" },
    { label: "DeepSeek Reasoner (R1)", value: "deepseek/deepseek-reasoner", note: "reasoning" }
  ],
  groq: [
    { label: "Llama 3.3 70B", value: "groq/llama-3.3-70b-versatile", note: "default" },
    { label: "Llama 3.1 8B", value: "groq/llama-3.1-8b-instant", note: "fast, cheap" },
    { label: "GPT-OSS 120B", value: "groq/openai/gpt-oss-120b" },
    { label: "Kimi K2", value: "groq/moonshotai/kimi-k2-instruct" }
  ],
  xai: [
    { label: "Grok 4", value: "xai/grok-4", note: "default" },
    { label: "Grok 3", value: "xai/grok-3" },
    { label: "Grok 3 mini", value: "xai/grok-3-mini", note: "fast, cheap" },
    { label: "Grok Code Fast", value: "xai/grok-code-fast-1", note: "coding" }
  ],
  mistral: [
    { label: "Mistral Large", value: "mistral/mistral-large-latest", note: "default" },
    { label: "Mistral Medium", value: "mistral/mistral-medium-latest" },
    { label: "Mistral Small", value: "mistral/mistral-small-latest", note: "fast, cheap" },
    { label: "Magistral Medium", value: "mistral/magistral-medium-latest", note: "reasoning" }
  ]
};
...
${hex("#10B981", "Recommended baseline:")}
  ...
  ${hex("#6B7280", "•")} ${c.bold("AGENT_API_TOKEN")} is required, including on localhost; setup generates it.
`,
    { accent: "#F59E0B" }
  );
}
...
${hex("#A855F7", "Install options:")}
  ...
  --provider NAME        ${c.dim("kimi|ollama|openrouter|openai|anthropic|gemini|deepseek|groq|xai|mistral|vllm")}
...
${hex("#A855F7", "Examples:")}
  ${c.dim("$")} distill
  ${c.dim("$")} npx @aspct/distill-agent install
  ${c.dim("$")} npx @aspct/distill-agent doctor
  ${c.dim("$")} npx @aspct/distill-agent logs --tail 100 --follow
`;
}
...
function run(command, args = [], options = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // Drain piped output while retaining a bounded tail for failed commands.
    // Unread pipes can fill during dependency installs and block the child.
    let output = "";
    for (const stream of [child.stdout, child.stderr]) {
      if (stream) {
        stream.setEncoding("utf8");
        stream.on("data", (chunk) => { output = (output + chunk).slice(-16384); });
      }
    }
    ...
    child.on("close", (code, signal) => {
      ...
      else reject(new Error(`${printable} exited with ${signal ? `signal ${signal}` : `code ${code}`}${output.trim() ? `\n${output.trim()}` : ""}`));
    });
  });
}
...
function defaultProviderSettings(provider) {
  switch (provider) {
    ...
    case "groq":
      return {
        agentModel: "groq/llama-3.3-70b-versatile",
        fastModel: "groq/llama-3.3-70b-versatile",
        strongModel: "groq/llama-3.3-70b-versatile",
        groqKey: ""
      };
    case "xai":
      return {
        agentModel: "xai/grok-4",
        fastModel: "xai/grok-4",
        strongModel: "xai/grok-4",
        xaiKey: ""
      };
    case "mistral":
      return {
        agentModel: "mistral/mistral-large-latest",
        fastModel: "mistral/mistral-large-latest",
        strongModel: "mistral/mistral-large-latest",
        mistralKey: ""
      };
    case "vllm":
      return {
...
async function collectProviderSettings(prompter, provider, mode) {
  ...
  if (provider === "kimi") {
    ...
    settings.deepseekKey = await prompter.question("DeepSeek API key", defaults.deepseekKey, { secret: true });
  } else if (provider === "groq") {
    settings.groqKey = await prompter.question("Groq API key", defaults.groqKey, { secret: true });
  } else if (provider === "xai") {
    settings.xaiKey = await prompter.question("xAI API key", defaults.xaiKey, { secret: true });
  } else if (provider === "mistral") {
    settings.mistralKey = await prompter.question("Mistral API key", defaults.mistralKey, { secret: true });
...
async function collectMessagingSettings(prompter, messaging) {
  const settings = {
    telegramToken: "",
  ...
  if (wantsTelegram) {
    ...
    settings.telegramAllowed = await prompter.question("Telegram allowed chat IDs (comma-separated; blank blocks messages)", settings.telegramAllowed);
  }
  if (wantsDiscord) {
    settings.discordToken = await prompter.question("Discord bot token", settings.discordToken, { secret: true });
    settings.discordAllowed = await prompter.question("Discord allowed user IDs (comma-separated; blank blocks messages)", settings.discordAllowed);
  }
  if (wantsSlack) {
    ...
    settings.slackAllowed = await prompter.question("Slack allowed user IDs (comma-separated; blank blocks messages)", settings.slackAllowed);
  }
  if (wantsEmail) {
    ...
    settings.emailAllowed = await prompter.question("Allowed sender addresses (comma-separated; blank blocks messages)", settings.emailAllowed);
  }
  return settings;
}
...
async function collectInstallConfig(options) {
  ...
  provider = normalizeChoice(await prompter.select(
    ...
    [
      ...
      { label: "OpenAI", value: "openai", note: "API key or ChatGPT sign-in" },
      { label: "Anthropic", value: "anthropic" },
      { label: "Google Gemini", value: "gemini" },
      { label: "DeepSeek", value: "deepseek" },
      { label: "Groq", value: "groq", note: "fast open-model inference" },
  ...
  const messagingSettings = await collectMessagingSettings(prompter, messaging);
...
function generateEnv(config) {
  ...
  return [
    ...
    envLine("DEEPSEEK_API_KEY", provider.deepseekKey || ""),
    envLine("GROQ_API_KEY", provider.groqKey || ""),
    envLine("XAI_API_KEY", provider.xaiKey || ""),
    envLine("MISTRAL_API_KEY", provider.mistralKey || ""),
...
function writeEnvFile(config) {
  ...
  if (config.dryRun) {
    ...
    logStep("[dry-run] generate AGENT_API_TOKEN if no existing token is configured");
    return;
  }
  ...
  let content = preserved.length ? `${preserved.join("\n")}\n\n${generated}` : generated;
  if (!hasEnvSecret(content, "AGENT_API_TOKEN")) {
    content += `AGENT_API_TOKEN=${randomBytes(32).toString("hex")}\n`;
  }
...
function missingCredential(config) {
  ...
  if (config.provider === "deepseek" && !provider.deepseekKey) return "DEEPSEEK_API_KEY";
  if (config.provider === "groq" && !provider.groqKey) return "GROQ_API_KEY";
  if (config.provider === "xai" && !provider.xaiKey) return "XAI_API_KEY";
  if (config.provider === "mistral" && !provider.mistralKey) return "MISTRAL_API_KEY";
  return "";
}
...
async function assertPortsAvailableForLocal(config) {
  ...
  throw new Error(
    ...
    "instance instead:\n" +
    "  npx @aspct/distill-agent restart\n" +
    "or stop it first:\n" +
    "  npx @aspct/distill-agent stop"
  );
}

function hasEnvSecret(content, key) {
  const assignment = new RegExp(`^\\s*(?:export\\s+)?${key}\\s*=([^\\r\\n]*)`, "gm");
  const matches = [...content.matchAll(assignment)];
  const value = matches.length ? matches[matches.length - 1][1].trim() : "";
  return Boolean(value && !/^(?:''|"")?\s*(?:#.*)?$/.test(value));
}

function ensureNeo4jPassword(config) {
  if (process.env.NEO4J_PASSWORD) return;
  const envPath = path.join(config.installDir, ".env");
  const existing = fs.existsSync(envPath) ? fs.readFileSync(envPath, "utf8") : "";
  const assignment = /^\s*(?:export\s+)?NEO4J_PASSWORD\s*=([^\r\n]*)/gm;
  if (hasEnvSecret(existing, "NEO4J_PASSWORD")) return;
  if (config.dryRun) {
    logStep(`[dry-run] generate NEO4J_PASSWORD in ${envPath}`);
    return;
  }
  const preserved = existing.replace(assignment, "");
  const separator = preserved && !preserved.endsWith("\n") ? "\n" : "";
  fs.writeFileSync(envPath, `${preserved}${separator}NEO4J_PASSWORD=${randomBytes(24).toString("hex")}\n`, { encoding: "utf8", mode: 0o600 });
  logStep(`Generated NEO4J_PASSWORD in ${envPath}`);
}

async function startServices(config) {
  if (config.sandbox === "on") {
    ensureNeo4jPassword(config);
  ...
  console.log(box("Distill is Ready", [
    ...
    `${c.dim("Manage with:")}`,
    `  ${c.dim("$")} distill`,
    `  ${c.dim("$")} npx @aspct/distill-agent stop`,
    `  ${c.dim("$")} npx @aspct/distill-agent logs --follow`,
    `  ${c.dim("$")} npx @aspct/distill-agent doctor`,
    ""
  ].join("\n"), { accent: "#10B981" }));
}
...
async function install(options) {
  ...
  console.log(`  Copy AGENT_API_TOKEN from ${path.join(config.installDir, "an-api.env")} into Control Panel Settings.`);
  ...
  if (!config.noStart) {
    ...
    console.log(`  ${c.dim("Start later with:")} npx @aspct/distill-agent start\n`);
  }
